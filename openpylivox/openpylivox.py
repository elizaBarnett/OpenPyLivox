...
class _dataCaptureThread(object):
    ...
    def __init__(self, sensorIP, data_socket, imu_socket, filePathAndName, fileType, secsToWait, duration, firmwareType, showMessages, format_spaces, deviceType):
        ...
        #pick if it's going to be bin, csv, etc.
        if self.fileType == 1:
    ...
    def run(self):
        ...
        while True:
            ...
            if self.started:
                ...
                if selectTest[0]:
                    data_pc, addr = self.d_socket.recvfrom(1500) # ok so this is the sample data package  which is 60 bytes long for us 
                    version = int.from_bytes(data_pc[0:1], byteorder='little') #byte 0 is version
                    self.dataType = int.from_bytes(data_pc[9:10], byteorder='little') #byte 9 is datatype
                    timestamp_type = int.from_bytes(data_pc[8:9], byteorder='little') #byte 8 is timestamp type
                    timestamp1 = self.getTimestamp(data_pc[10:18], timestamp_type) #bytes 10 through 17 are the timestamp
                    data1 = data_pc[18:] 
                    print("first point", data1)
                    self.updateStatus(data_pc[4:8]) #status code 
                    if self.isCapturing:
        ...
        if breakByCapture:
            ...
            if version == 5:
                        ...
                        if timeSinceStart <= self.duration:
                            ...
                            if select.select([self.d_socket], [], [], 0)[0]:
                                ...
                                if self.firmwareType == 1:
                                    ...
                                    # Horizon, Tele-15, and Cartesian (single return)
                                    elif dataType == 2:
                                        # to account for first point's timestamp being increment in the loop
                                    ...
                                    # Horizon, Tele-15, and Avia Spherical (single return)
                                    elif dataType == 3:
                                        # to account for first point's timestamp being increment in the loop
                                        timestamp_sec -= 0.000004167
                                        for i in range(0, 96):
                                    ...
                                    # Horizon, Tele-15 and Avia Cartesian (dual return)
                                    elif dataType == 4:
                                        # to account for first point's timestamp being increment in the loop
                                    ...
                                    # Horizon, Tele-15, and Avia Spherical (dual return)
                                    elif dataType == 5:
                                        # to account for first point's timestamp being increment in the loop
                                        timestamp_sec -= 0.000002083
                                        for i in range(0, 48):
                ...
                if self._showMessages:
                    ...
                    print("                                (points: " + str(numPts) + " good, " + str(nullPts) + " null, " + str(numPts + nullPts) + " total)")
                    if self._deviceType == "Horizon" or self._deviceType == "Tele-15" or self.deviceType == "Avia":
                        print("                                (IMU records: " + str(imu_records) + ")")

                binFile.close()
                RawBin.write(data_pc)
                RawBin.close()
...
class openpylivox(object):
    ...
    _CMD_GET_LIDAR_RETURN_MODE =  bytes.fromhex((b'AA010F0000000004D70107604ffb12').decode('ascii')) 
    ...
    _CMD_LIDAR_TRIPLE =           bytes.fromhex((b'AA011000000000B809010603BBE4A9DA').decode('ascii'))  
    ...
    def _reinit(self):
        ...
        #sockets are apparently endpoints for sending/recieving data 
    ...
    def _lidarSpinUp(self):
        ...
        if self._isConnected:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                binData, addr = self._cmdSocket.recvfrom(16) #Common Error place 
                _, ack, cmd_set, cmd_id, ret_code_bin = self._parseResp(binData)
    ...
    def _dataStart_RT_B(self):
        ...
        if self._isConnected:
            if not self._isData: #boolean whether or not the datastream is currently being recorded 
                ...
                time.sleep(0.12)
                self._waitForIdle()
                self._cmdSocket.sendto(self._CMD_DATA_START, (self._sensorIP, 65000)) #sends binary command to start 
                if self._showMessages: print("   " + self._sensorIP + self._format_spaces + "   <--     sent start data stream request")

                # check for proper response from data start request
                if select.select([self._cmdSocket], [], [], 0.1)[0]: #you get a response depending on the the command you sned out 
                    binData, addr = self._cmdSocket.recvfrom(16)
                    _, ack, cmd_set, cmd_id, ret_code_bin = self._parseResp(binData)
    ...
    def setLidarReturnMode(self, Mode_ID):
        ...
        if self._isConnected:
            ...
            # triple returns
                ...
                self._cmdSocket.sendto(self._CMD_LIDAR_TRIPLE, (self._sensorIP, 65000))
                if self._showMessages: print("   " + self._sensorIP + self._format_spaces + "   <--     sent triple return lidar mode request")
    ...
    def _getReturnMode(self):  

        if self._isConnected:
            self._waitForIdle()

            self._cmdSocket.sendto(self._CMD_GET_LIDAR_RETURN_MODE, (self._sensorIP, 65000))
            if self._showMessages: print("   " + self._sensorIP + self._format_spaces + "   <--     sent get return mode state request")
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "Lidar" and cmd_id == "7":
                    ret_code = int.from_bytes(ret_code_bin[0], byteorder='little')
                    if ret_code == 1:
                        if self._showMessages: print(
                            "   " + self._sensorIP + self._format_spaces + "   -->     FAILED to get lidar return value")
                    elif ret_code == 0:
                        value = struct.unpack('<B', binData[12:13])[0]
                        print("   " + self._sensorIP + self._format_spaces + "   -->      Returns: " + str(value))
                else:
                    if self._showMessages: print(
                        "   " + self._sensorIP + self._format_spaces + "   -->     incorrect get return mode response")
        else:
            if self._showMessages: print("Not connected to Livox sensor at IP: " + self._sensorIP)
                        ...
                        if dataType >= 0 and dataType <= 7: 
                            ...
                            while True:
                                try:
                                    ...
                                    # Avia Cartesian triple return (SDK Data Type 7) 
                                    elif dataClass == 9: 
                                        coord1a = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord2a = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord3a = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        ...
                                        #pretty sure all points after this are null 
                                        coord1b = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord2b = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        coord3b = float(struct.unpack('<i', binFile.read(4))[0]) / 1000.0
                                        intensityb = struct.unpack('<B', binFile.read(1))[0]
                                        ...
                                        spatial_confb = str(int(tag_bitsb[0:2], 2))
                                        intensity_confb = str(int(tag_bitsb[2:4], 2))
                                        returnTypeb = str(int(tag_bitsb[4:6], 2)) #also returns 0 with print(returnTypeb)
                                        ...
                                        spatial_confc = str(int(tag_bitsc[0:2], 2))
                                        intensity_confc = str(int(tag_bitsc[2:4], 2))
                                        returnTypec = str(int(tag_bitsc[4:6], 2)) #it is getting here, checked in w/ print(returnTypec) that returned a bunch of zeros though
                                        ...
                                        csvFile.write("{0:.3f}".format(coord1c) + "," + "{0:.3f}".format(
                                            coord2c) + "," + "{0:.3f}".format(coord3c) + "," + str(
                                            intensityc) + "," + "{0:.6f}".format(timestamp_sec) + ",3," + returnTypec + "," ##forgot to change 2 to 3 lmao
                                                      + spatial_confc + "," + intensity_confc + "\n")
                                    pbari.update(1)
                ...
                if firmwareType >= 1 and firmwareType <= 3:
                    ...
                    if dataType == 0 or dataType == 2 or dataType == 4: 
                        ...
                        if firmwareType == 1 and dataType == 0:
                            ...
                            print("ConvertBin2Las pt 1 Avia working (:")
                            dataClass = 9
                            divisor = 50 
